...
(define customHT (make-hash))
(hash-set! customHT "k1" 1)
(hash-set! customHT "k2" 2)
(hash-set! customHT "k3" 3)
(hash-set! customHT "k4" 4)
(hash-set! customHT "a1" .5)
(hash-set! customHT "a2" .25)
(hash-set! customHT "a3" .15)
(hash-set! customHT "a4" .1)
(hash-set! customHT "phase1" 0)
(hash-set! customHT "phase2" .1)
(hash-set! customHT "phase3" .2)
(hash-set! customHT "phase4" 0)

(define (get-timbre-hash timbre)
  (cond ((eq? timbre 'piano) pianoHT)
        ((eq? timbre 'clarinet) clarinetHT)
        ((eq? timbre 'custom) customHT)
        (else pianoHT)))
